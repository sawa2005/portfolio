...
function sassTask() {
  return src(files.sassPath).pipe(sass().on("error", sass.logError)).pipe(dest("src/css"));
}
...
function copyData() {
  return src(files.dataPath).pipe(dest("pub/data"));
}
...
function jsTask() {
  return src(files.jsPath).pipe(concat("main.js")).pipe(terser()).pipe(dest("pub/js"));
}

// Image-task / Kopierar bildfilerna
function imgTask() {
  return src(files.imgPath, { encoding: false }).pipe(dest("pub/imgs", { overwrite: true }));
}
...
exports.build = series(copyHTML, copyData, tsTask, jsTask, imgTask, sassTask, cssTask);
